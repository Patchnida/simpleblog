...
const LoginForm = () => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        const userProfileRes = await fetch("/api/user", {
          method: "GET",
          headers: {
        ...
        if (!userProfile.user.name || !userProfile.user.surname || !userProfile.user.phone) {
    ...
    return (
      <div className="w-full min-w-80 lg:max-w-2xl">
        <form
          ...
          className="flex flex-col justify-center items-center bg-white w-full px-5 md:px-10 py-14 md:py-36 lg:py-16 gap-5 md:gap-10 lg:gap-7 shadow"
        >
          ...
          <div>
            <p className="text-xs sm:text-sm md:text-base font-normal">
              ...
              <Link href="/register" className="font-semibold ml-1 hover:underline hover:text-blue-500">
                Register
              </Link>
            </p>
          </div>
